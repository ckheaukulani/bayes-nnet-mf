...
class VIBinaryNNetMF:
    ...
    def build(self):
        ...
        U = tf.Variable(tf.random_normal([N, n_factors]), name='U')
        Up = tf.Variable(tf.random_normal([N, d_pairwise]), name='Up')

        inputs_ = tf.concat([tf.gather(U, indices=self.row),
                             tf.gather(U, indices=self.col),
                             tf.gather(Up, indices=self.row) * tf.gather(Up, indices=self.col)
        ...
        inputs_ = tf.tile(inputs_[None, :, :], [self.n_samples, 1, 1])  # this is probably very inefficient
